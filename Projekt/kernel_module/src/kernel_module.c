...
static int parse_scientific(const char *buf, u64 *val)
{
    ...
    exp10 -= frac_digits;
    if (mantissa == 0) { *val = 0; return 0; }

    u64 m = mantissa;
    int exp2 = 0;

    while (m >= (HIDDEN_BIT << 1)) { m >>= 1; exp2++; }
    while (m < HIDDEN_BIT && m > 0) { m <<= 1; exp2--; }

    int exp2_adjust = 0;
    int e10 = exp10;
    if (e10 > 0) {
        while (e10 > 0) { exp2_adjust += 3; e10--; }
    } else {
        while (e10 < 0) { exp2_adjust -= 3; e10++; }
    }
    exp2 += exp2_adjust;
...
static int format_scientific(u64 val, char *buf, size_t len)
{
    ...
    if (exp == 0) return snprintf(buf, len, "0.0");
    if (exp == (1 << EXP_BITS) - 1) {
...
static ssize_t ctstma_write(struct file *f, const char __user *ubuf, size_t c, loff_t *pos)
{
    ...
    if (cmd == 1) {
        ...
        udelay(10);
        u32 ctrl_val = readl(baseptr + OFF_CTRL);
        u32 stat_val = readl(baseptr + OFF_STATUS);
        pr_info("SYKOM: after write: ctrl=0x%08x, status=0x%08x\n", ctrl_val, stat_val);
    } else if (cmd == 0) {
...
static ssize_t restma_read(struct file *f, char __user *ubuf, size_t c, loff_t *pos)
{
    ...
    u32 st = readl(baseptr + OFF_STATUS);
    if (!(st & STATUS_DONE)) {
        if (st & STATUS_BUSY) len = snprintf(buf, sizeof(buf), "busy\n");
        else if (st & STATUS_ERROR) len = snprintf(buf, sizeof(buf), "error\n");
        else len = snprintf(buf, sizeof(buf), "idle\n");
    } else {
...
static const struct file_operations a1_fops = { .owner = THIS_MODULE, .write = a1stma_write };
static const struct file_operations a2_fops = { .owner = THIS_MODULE, .write = a2stma_write };
static const struct file_operations ctrl_fops = { .owner = THIS_MODULE, .write = ctstma_write };
static const struct file_operations stat_fops = { .owner = THIS_MODULE, .read = ststma_read };
static const struct file_operations res_fops = { .owner = THIS_MODULE, .read = restma_read };

static int __init sykom_init(void)
{
    pr_info("SYKOM: Initializing module\n");
    baseptr = ioremap(SYKT_GPIO_BASE_ADDR, SYKT_GPIO_SIZE);
    if (!baseptr) { pr_err("SYKOM: ioremap failed\n"); return -ENOMEM; }
